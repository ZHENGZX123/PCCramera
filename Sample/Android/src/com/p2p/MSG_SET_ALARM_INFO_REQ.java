...
public class MSG_SET_ALARM_INFO_REQ {
	...
	public boolean get15MinutesFlag(int nWeek, int nBitIndex){
		if(nBitIndex<0 || nBitIndex>=96) return false;
		int nBitValue=0;
		switch(nWeek){
			case 0:{// sunday
				if(nBitIndex<32){
					nBitValue=(nAlarmTime_sun_0>>nBitIndex) & 0x01;
				}else if(nBitIndex<63){
					nBitValue=(nAlarmTime_sun_1>>nBitIndex) & 0x01;
				}else if(nBitIndex<95){
					nBitValue=(nAlarmTime_sun_2>>nBitIndex) & 0x01;
				}
				}break;
