...
public class MSG_SET_CAMERA_PARAM_REQ {
	public static final int MY_LEN = 56+60;
	...
	*/
	public static byte[] getBytes(int nResolution, int nBright, int nContrast,
			int nSaturation, int nMode, int nFlip, int nIRLed, int nBitMaskToSet, 
			int bOSDTime, int bOSDName, int bOSDTempHumi, String strOSDName) 
	{
		...
		byte[] bOSDEnable =new byte[4];
		Arrays.fill(bOSDEnable, (byte)0);
		bOSDEnable[0]=(byte)(bOSDTime);
		bOSDEnable[1]=(byte)(bOSDName);
		bOSDEnable[2]=(byte)(bOSDTempHumi);
		System.arraycopy(bOSDEnable, 0, byts, 20, bOSDEnable.length);
